...
export const Sidebar: React.FC<{
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-white/[0.08] bg-[#080C14] flex flex-col justify-between shrink-0 h-screen transform transition-transform duration-200 ease-out md:sticky md:top-0 md:translate-x-0 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
      <div>
        <div className="h-16 px-6 flex items-center justify-between gap-3 border-b border-white/[0.08]">
          ...
          <button
            onClick={onClose}
            className="md:hidden w-8 h-8 rounded-lg bg-slate-900/60 border border-white/5 text-slate-400 hover:text-white flex items-center justify-center shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
